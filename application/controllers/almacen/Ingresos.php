...
class Ingresos extends CI_Controller {
	...
	public function lista(){
		if ($this->input->is_ajax_request()) {
			...
			$lista = $this->db->query("select kardex.codkardex, kardex.codcomprobantetipo, kardex.seriecomprobante,kardex.nrocomprobante, kardex.fechakardex,round(kardex.importe,2) as importe,kardex.seriecomprobante_ref,kardex.nrocomprobante_ref, usuarios.usuario,tipos.descripcion as tipomovimiento,comprobantes.descripcion as tipo,kardex.estado from kardex.kardex as kardex inner join seguridad.usuarios as usuarios on (kardex.codusuario=usuarios.codusuario) inner join caja.comprobantetipos as comprobantes on(kardex.codcomprobantetipo_ref=comprobantes.codcomprobantetipo) inner join almacen.movimientotipos as tipos on(kardex.codmovimientotipo=tipos.codmovimientotipo) where codalmacen=".$_SESSION["netix_codalmacen"]." and (UPPER(usuarios.usuario) like UPPER('%".$this->request->buscar."%') or UPPER(tipos.descripcion) like UPPER('%".$this->request->buscar."%') or UPPER(kardex.seriecomprobante) like UPPER('%".$this->request->buscar."%') or UPPER(kardex.nrocomprobante) like UPPER('%".$this->request->buscar."%') ) and tipos.tipo=1 and kardex.codmovimientotipo<>2 order by kardex.codkardex desc offset ".$offset." limit ".$limit)->result_array();
			$total = $this->db->query("select count(*) as total from kardex.kardex as kardex inner join seguridad.usuarios as usuarios on (kardex.codusuario=usuarios.codusuario) inner join caja.comprobantetipos as comprobantes on(kardex.codcomprobantetipo_ref=comprobantes.codcomprobantetipo) inner join almacen.movimientotipos as tipos on(kardex.codmovimientotipo=tipos.codmovimientotipo) where codalmacen=".$_SESSION["netix_codalmacen"]." and (UPPER(usuarios.usuario) like UPPER('%".$this->request->buscar."%') or UPPER(tipos.descripcion) like UPPER('%".$this->request->buscar."%') or UPPER(kardex.seriecomprobante) like UPPER('%".$this->request->buscar."%') or UPPER(kardex.nrocomprobante) like UPPER('%".$this->request->buscar."%') ) and tipos.tipo=1 and kardex.codmovimientotipo<>2")->result_array();
	...
	function transferencias(){
		if ($this->input->is_ajax_request()) {
			$transferencias = $this->db->query("select k.codkardex,k.fechakardex,k.seriecomprobante,k.nrocomprobante,k.seriecomprobante_ref, k.nrocomprobante_ref,round(k.importe,2) as importe, a.descripcion as almacen from kardex.kardex as k inner join almacen.almacenes as a on(k.codalmacen=a.codalmacen) where k.codalmacen_ref=".$_SESSION["netix_codalmacen"]." and k.codkardex_ref is null and k.transferido=0 and k.estado=1")->result_array();
	...
	function guardar_transferencia(){
		if ($this->input->is_ajax_request()) {
			if (isset( $_SESSION["netix_codusuario"]) ) {
				...
				$series = $this->db->query("select seriecomprobante from caja.comprobantes where codcomprobantetipo=3 and codsucursal=".$_SESSION["netix_codsucursal"]." and codalmacen=".$_SESSION["netix_codalmacen"]." and estado=1")->result_array();
				...
				$valores = [
					...
					(int)$_SESSION["netix_codusuario"],12,date("Y-m-d"),date("Y-m-d"),3,
				...
				$valores = [
					...
					(int)$codkardex,
					(int)$_SESSION["netix_codusuario"],12,date("Y-m-d"),3,
					$series[0]["seriecomprobante"]
				];
				$codkardexalmacen = $this->Netix_model->netix_guardar("kardex.kardexalmacen", $campos, $valores, "true");
				$nro_comprobante = $this->Kardex_model->netix_kardexcorrelativo($codkardex,$codkardexalmacen,3,$series[0]["seriecomprobante"]);
				...
				foreach ($this->request->detalle as $key => $value) { $item = $item + 1;
					...
					$valores =[
						...
						(int)$this->request->detalle[$key]->codproducto,
						(int)$this->request->detalle[$key]->codunidad,
						(int)$_SESSION["netix_codalmacen"], $item,
						(int)$_SESSION["netix_codsucursal"],
						(double)$this->request->detalle[$key]->cantidad
					];
					$estado = $this->Netix_model->netix_guardar("kardex.kardexalmacendetalle", $campos, $valores);
				...
				$campos = ["valorventa","porcigv","igv","importe"]; $valores = [$subtotal,$_SESSION["netix_igv"],$igv,$total];
				$estado = $this->Netix_model->netix_editar("kardex.kardex", $campos, $valores, "codkardex", $codkardex);

				$campos = ["transferido"]; $valores = [1];
				$estado = $this->Netix_model->netix_editar("kardex.kardex", $campos, $valores, "codkardex", $this->request->kardex_ref);
