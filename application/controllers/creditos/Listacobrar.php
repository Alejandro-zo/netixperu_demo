...
class Listacobrar extends CI_Controller {
	...
	public function lista(){
		if ($this->input->is_ajax_request()) {
			...
			$lista = $this->db->query("select personas.razonsocial,personas.documento,creditos.codcredito,creditos.codkardex, creditos.fechacredito, creditos.fechavencimiento, round(creditos.importe,2) as importe, round(creditos.interes,2) as interes, round(creditos.saldo,2) as saldo, creditos.estado from kardex.creditos as creditos inner join public.personas as personas on (creditos.codpersona=personas.codpersona) where (UPPER(personas.documento) like UPPER('%".$this->request->buscar."%') or UPPER(personas.razonsocial) like UPPER('%".$this->request->buscar."%') or UPPER(personas.nombrecomercial) like UPPER('%".$this->request->buscar."%') ) and creditos.codsucursal=".$_SESSION["netix_codsucursal"]." and creditos.estado<>0 and creditos.tipo=1 order by creditos.codcredito desc offset ".$offset." limit ".$limit)->result_array();
			...
			$total = $this->db->query("select count(*) as total from kardex.creditos as creditos inner join public.personas as personas on (creditos.codpersona=personas.codpersona) where (UPPER(personas.documento) like UPPER('%".$this->request->buscar."%') or UPPER(personas.razonsocial) like UPPER('%".$this->request->buscar."%') or UPPER(personas.nombrecomercial) like UPPER('%".$this->request->buscar."%') ) and creditos.codsucursal=".$_SESSION["netix_codsucursal"]." and creditos.estado<>0 and creditos.tipo=1")->result_array();
	...
	function pdf_creditos(){
		if (isset($_SESSION["netix_usuario"])) {
		    ...
		    $lista = $this->db->query("select personas.razonsocial,personas.documento,creditos.* from kardex.creditos AS creditos inner join public.personas as personas on (creditos.codpersona=personas.codpersona) where creditos.codsucursal=".$_SESSION["netix_codsucursal"]." and creditos.estado=1 and creditos.tipo=1 order by creditos.codcredito desc")->result_array();
			...
			$pdf->SetTitle(utf8_decode("Netix Perú - Creditos Cobrar")); $pdf->Output();
		}else{
			$this->load->view("netix/505");
		}
	}
