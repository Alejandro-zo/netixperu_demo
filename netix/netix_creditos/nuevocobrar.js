var netix_nuevocredito = new Vue({
	...
	data: {
		campos:{
			"codregistro":"","codpersona":netix_creditos.registro,"codempleado":0,"fechacredito":$("#fecha").val(),"fechainicio":$("#fecha").val(),
			"nrodias":30,"nrocuotas":1,"codcreditoconcepto":1,"codcajaconcepto":7,"codtipopago":1,"fechadocbanco":$("#fechadocbanco_ref").val(),
			"nrodocbanco":"","importe":"","tasainteres":0,"interes":0,"total":0,"tipocuota":0,"afectacaja":true,"referencia":"","codpersona_convenio":1
		},
		estado:0, cuotas: []
	},
	methods: {
		...
		netix_guardar: function(){
			...
			this.estado = 1; netix_sistema.netix_inicio_guardar("GUARDANDO CREDITO POR COBRAR . . .");
			this.$http.post(url+netix_controller+"/guardar", {"campos":this.campos,"cuotas":this.cuotas}).then(function(data){
				if (data.body=="e") {
					...
					if (data.body==1) {
						netix_sistema.netix_alerta("CREDITO POR COBRAR REGISTRADO","CREDITO REGISTRADO EN EL SISTEMA","success");
					}else{
				...
				netix_sistema.netix_alerta("ERROR AL CUENTA POR COBRAR","ERROR DE RED","error");
			});
		},
