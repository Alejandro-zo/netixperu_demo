var netix_cobranza = new Vue({
	...
	data: {
		campos:{
			"codpersona":netix_creditos.registro,"codconcepto":17,"codcomprobantetipo":2,"codtipopago":1,"importe":0,"vuelto":0,
			"fechadocbanco":$("#fechadocbanco").val(),"nrodocbanco":"","total":0,"descripcion":"PAGO DE CUOTAS"
		},
	...
	methods: {
		...
		netix_cobrar: function(index,cuota){
			if ($("#"+index).is(":checked")){
				...
				this.campos.total = Number(( (this.campos.total + parseFloat(cuota.saldo)) ).toFixed(2)); this.netix_vuelto();
			}
		},
		...
		netix_guardar: function(){
			...
			this.estado = 1; netix_sistema.netix_inicio_guardar("GUARDANDO PAGO DEL CREDITO . . .");
			this.$http.post(url+netix_controller+"/pagar", {"campos":this.campos,"cuotas":this.cuotascobrar}).then(function(data){
				if (data.body=="e") {
					...
					if (data.body==1) {
						netix_sistema.netix_alerta("PAGO REGISTRADO","CUOTA DE CREDITO PAGADO EN EL SISTEMA","success");
					}else{
						netix_sistema.netix_alerta("ERROR AL REGISTRAR PAGO","ERROR DE RED","error");
					}
				}
				netix_sistema.netix_fin(); netix_sistema.netix_modulo();
			}, function(){
				netix_sistema.netix_alerta("ERROR AL REGISTRAR PAGO","ERROR DE RED","error"); netix_sistema.netix_fin();
			});
		},
