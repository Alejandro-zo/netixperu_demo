	...
	methods: {
		...
		netix_guardar: function(){
			if (this.detalle.length==0) {
				netix_sistema.netix_noti("REGISTRAR UN PRODUCTO EN EL DETALLE","REGISTRAR ITEM PARA LA SALIDA","error"); 
				return false;
			}

			this.campos.fechakardex = $("#fechakardex").val();
			this.estado = 1; netix_sistema.netix_inicio_guardar("GUARDANDO SALIDA DE ALMACEN . . .");
			...
			this.$http.post(url+netix_controller+"/guardar", {"campos":this.campos,"detalle":this.detalle,"totales":this.totales}).then(function(data){
				if (data.body=="e") {
					...
					if (data.body==1) {
						netix_sistema.netix_alerta("SALIDA DE ALMACEN REGISTRADO","SALIDA DE ALMACEN EN EL SISTEMA","success");
					}else{
						netix_sistema.netix_alerta("ERROR AL REGISTRAR SALIDA DE ALMACEN","ERROR DE RED","error");
					}
				}
